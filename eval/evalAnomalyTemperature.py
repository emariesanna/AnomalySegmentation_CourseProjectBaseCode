...
def main(MyPath = './Dataset/Validation_Dataset/RoadObsticle21/images/*.webp', MyTemperature = None):
    ...
    for path in glob.glob(os.path.expanduser(myinput)):
        ...
        #anomaly_result =  1-np.max(probabilities.squeeze(0).data.cpu().numpy(), axis=0)   
        #print(result)
    ...
    file.write('\n Temperature: ' + str(model.temperature.item())+ '\n')
